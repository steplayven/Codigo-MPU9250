...
float coordenadasactualesY(){
  //angulo en radianes
  float angulor = radians(angulo_de_salida);
  //se le saca el sen al angulo
  float SenAngulo = sin(angulor);
  float hipotenusa = posicionLR();
  float Y;

  //Y seria el cateto opuesto del triangulo que se forma en el plano por lo tanto la formula seria
  Y = hipotenusa * SenAngulo;

  return Y;
}
float coordenadasactualesX(){
  //Datos necesarios de otras funciones
  float hipotenusa = posicionLR();
  float Y = coordenadasactualesY();
  float X;

  //X es el cateto que sobra, osea la h al cuadrado - el cateto al cuadrado todo en una raiz cuadrada
  X = sqrt(pow(hipotenusa,2)-pow(Y,2));
